 ...
 * In our sample we use it as a post-processing class for Faster-RCNN networks
 */
class DetectionOutputPostProcessor {
    CNNLayer cnnLayer;
...
public:
    explicit DetectionOutputPostProcessor(const CNNLayer* layer): cnnLayer(*layer) {
        try {
            ...
            auto prior_dims = cnnLayer.insData[idx_priors].lock()->dims;
            int priors_size = prior_dims[0]*prior_dims[1];
            auto loc_dims = cnnLayer.insData[idx_location].lock()->dims;
            int loc_size = loc_dims[loc_dims.size() - 2]*loc_dims[loc_dims.size() - 1];
            auto conf_dims = cnnLayer.insData[idx_confidence].lock()->dims;
