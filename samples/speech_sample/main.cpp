...
void GetKaldiArkInfo(const char *fileName,
    ...
    if (in_file.good()) {
        while (!in_file.eof()) {
            std::string line;
            uint32_t numRows, numCols, num_bytes;
...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
    ...
    if (FLAGS_i.empty() || (FLAGS_m.empty() && FLAGS_rg.empty())) {
        throw std::logic_error("Input file not set. Please use -i.");
    }

    if (FLAGS_i.empty() || (FLAGS_m.empty() && FLAGS_rg.empty())) {
        throw std::logic_error("One of required parameters is not set. Please use -h.");
    }
    ...
    if ((FLAGS_d.compare("GPU") != 0) && (FLAGS_d.compare("CPU") != 0) && (FLAGS_d.compare("GNA_AUTO") != 0) && (FLAGS_d.compare("GNA_HW") != 0)
...
int main(int argc, char *argv[]) {
    try {
        ...
        InferRequest request(executableNet.CreateInferRequest());

        float scaleFactorOutput = 1.0f;
        uint32_t numOutputs = 0;
        uint32_t numOutputsPadded = 0;
        // TO DO:  get network output size and padded output size
        bool isInputTransposed = true;
        bool isOutputTransposed = true;
        // -----------------------------------------------------------------------------------------------------
        ...
        Blob::Ptr ptrOutputBlob;
        ptrOutputBlob = request.GetBlob(cOutputInfo.begin()->first);
        ...
        std::vector<uint8_t> ptrInputBatch;
        std::vector<uint8_t> ptrScores;
        std::vector<uint8_t> ptrReferenceScores;
        std::vector<uint8_t> ptrOutputBatch;
        ...
        for (uint32_t utteranceIndex = 0; utteranceIndex < numUtterances; ++utteranceIndex) {
            ...
            if (ptrInputBlob->size() != numFrameElementsInputPadded * batchSize) {
                throw std::logic_error("network input size(" + std::to_string(ptrInputBlob->size()) +
                    ") mismatch to ark file size (" + std::to_string(numFrameElementsInputPadded * batchSize) + ")");
            }
            ptrInputBatch.resize(numFrameElementsInputPadded * batchSize * numBytesPerElementInput);
            ptrScores.resize(numFrames * numScoresPerFrame * sizeof(float));
            ptrOutputBatch.resize(ptrOutputBlob->size() * sizeof(float));
            if (!FLAGS_r.empty()) {
            ...
            uint32_t numBytesPerScore = (useGna) ? sizeof(int32_t) : sizeof(float);
            double totalTime = 0.0;

            std::cout << "Utterance " << utteranceIndex << ": " << uttName << std::endl;

            ClearScoreError(&totalError);
            totalError.threshold = frameError.threshold = MAX_SCORE_DIFFERENCE;
            for (uint32_t frameIndex = 0; frameIndex < numFrames; frameIndex += batchSize) {
                uint32_t
                    numFramesThisBatch = (numFrames - frameIndex < batchSize) ? (numFrames - frameIndex) : batchSize;

                memcpy(ptrInputBlob->buffer().as<char *>(),
                       &ptrUtterance.front() + frameIndex * sizeof(float) * numFrameElementsInput,
                       numFramesThisBatch * sizeof(float) * numFrameElementsInput);
                memset(ptrInputBlob->buffer().as<char *>() + numFramesThisBatch * sizeof(float) * numFrameElementsInput,
                       0,
                       (batchSize - numFramesThisBatch) * sizeof(float) * numFrameElementsInput);
                ...
                memcpy(&ptrScores.front() + frameIndex * numScoresPerFrame * sizeof(float),
                       ptrOutputBlob->buffer(),
                       numFramesThisBatch * numScoresPerFrame * sizeof(float));

                if (!FLAGS_r.empty()) {
                    uint32_t numErrors = CompareScores(ptrOutputBlob->buffer().as<float *>(),
                                                       &ptrReferenceScores[frameIndex * numFrameElementsReference
                                                               * numBytesPerElementReference],
                                                       &frameError,
                                                       numFramesThisBatch,
                                                       numFrameElementsReference);
            ...
            SaveKaldiArkArray(FLAGS_o.c_str(), shouldAppend, uttName, &ptrScores.front(), numFrames, numScoresPerFrame);

            /** Show performace results **/
            std::cout << "   Average inference time per frame: " << totalTime / static_cast<double>(numFrames) << " ms"
                      << std::endl;
            if (FLAGS_pc) {
                printPerformanceCountsPlugin(plugin, std::cout);
            }
            if (!FLAGS_r.empty()) {
                std::cout << "         max error: " << totalError.maxError << std::endl;
                std::cout << "         avg error: " << totalError.sumError / totalError.numScores << std::endl;
                std::cout << "     avg rms error: " << totalError.sumRmsError / numFrames << std::endl;
                std::cout << "       stdev error: " << StdDevError(totalError) << std::endl << std::endl;
            }
        }
        // -----------------------------------------------------------------------------------------------------
    }
