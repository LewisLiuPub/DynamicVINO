    ...
    try {
        ...
        for (auto & item : inputInfo) {
            ...
            for (auto & i : images) {
                ...
                /** Getting image data **/
                std::shared_ptr<unsigned char> data(reader->getData(item.second->getTensorDesc().getDims()[3], item.second->getTensorDesc().getDims()[2]));
                if (data.get() != nullptr) {
            ...
            /** Set the layout and precision of input data provided by the user,
        ...
        std::cout << std::endl << "Throughput: " << 1000 * static_cast<double>(FLAGS_ni) / total << " FPS" << std::endl;
        std::cout << std::endl;
        ...
        for (size_t i = 0; i < FLAGS_nireq; i++) {
            /** Validating -nt value **/
            const int resultsCnt = outputBlobs[i]->size() / network.getBatchSize();
