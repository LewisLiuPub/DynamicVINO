...
template <typename T>
void matU8ToBlob(const cv::Mat& orig_image, Blob::Ptr& blob, int batchIndex = 0) {
...
struct VehicleAttribsDetection : BaseDetection {
    ...
    void enqueue(const cv::Mat &Vehicle) {
        ...
        matU8ToBlob<uint8_t>(Vehicle, inputBlob);
    }
    ...
    Attributes GetAttributes() const {
        ...
        const auto colorsValues = request->GetBlob(outputNameForColor)->buffer().as<float*>();
        // 4 possible types for each vehicle and we should select the one with the maximum probability
        const auto typesValues  = request->GetBlob(outputNameForType)->buffer().as<float*>();
...
struct LPRDetection : BaseDetection {
    ...
    void enqueue(const cv::Mat &Plate) {
        if (!enabled())
            return;
        if (!request)
            request = net.CreateInferRequestPtr();

        Blob::Ptr inputBlob = request->GetBlob(inputImageName);
        matU8ToBlob<uint8_t>(Plate, inputBlob);
    ...
    std::string GetLicencePlateText() const {
            ...
            for (auto && result : VehicleDetection.results) {
                if (result.label == 1) {  // vehicle
                    if (VehicleAttribs.enabled()) {
                        ...
                        AttribsInferred++;
                        // ----------------------------Process outputs-----------------------------------------------------
                        VehicleAttribsDetection::Attributes res = VehicleAttribs.GetAttributes();
                        cv::putText(frame,
                                    res.color,
