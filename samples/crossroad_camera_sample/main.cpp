...
struct PersonReIdentification : BaseDetection {
    ...
    CNNNetwork read() override {
        ...
        inputInfoItem.second->setInputPrecision(Precision::U8);
        ...
        do {
            ...
            for (auto && result : personDetection.results) {
                if (result.label == 1) {  // person
                    ...
                    cv::Mat person = frame(clippedRect);
                    if (personAttribs.enabled()) {
                        ...
                        // --------------------------- Process outputs -----------------------------------------
                        std::string res = personAttribs.GetPersonAttributes();
                        cv::putText(frame,
                                    res,
                        ...
                        if (FLAGS_r) {
                            std::cout << "Person Attributes results:" << res << std::endl;
                        }
                    }
                    if (personReId.enabled()) {
                        ...
                        std::string res = "REID: " + std::to_string(foundId);
                        cv::putText(frame,
                                    res,
                        ...
                        if (FLAGS_r) {
                            std::cout << "Person Reidentification results:" << res << std::endl;
                        }
                    }
