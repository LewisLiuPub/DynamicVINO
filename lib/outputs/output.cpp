...
void
ImageWindow::prepareData(const InferenceResult::FaceDetectionResult &result) {
...
void ImageWindow::prepareData(const InferenceResult::EmotionsResult &result) {
  std::ostringstream out;
  cv::Rect rect = result.location;
...
void ImageWindow::prepareData(const InferenceResult::AgeGenderResult &result) {
  std::ostringstream out;
  cv::Rect rect = result.location;
...
void ImageWindow::prepareData(const InferenceResult::HeadPoseResult& result) {
  int scale  = 50;
